  ...
  module Services
    ...
    class BaseService
      ...
      def resolve_instrument_id(instrument_id:)
        return instrument_id if instrument_id && !instrument_id.to_s.empty?

        raise InvalidArgumentError, 'instrument_id is required'
      end

      def resolve_instrument_ids(instrument_id:)
        ids = instrument_id ? Array(instrument_id) : []
        ids = ids.reject { |id| id.to_s.empty? }
